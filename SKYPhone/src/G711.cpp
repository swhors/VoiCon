...
EXPORT uint8 G711::ALawEncode(int16 pcm16)
{
	int p = pcm16;
...
EXPORT int G711::ALawDecode(uint8 alaw)
{
...
EXPORT uint8 G711::ULawEncode(int16 pcm16)
{
	int p = pcm16;
...
EXPORT int G711::ULawDecode(uint8 ulaw)
{
...
EXPORT uint8 G711::ALawToULaw(uint8 alaw)
{
...
EXPORT uint8 G711::ULawToALaw(uint8 ulaw)
{
...
EXPORT uint G711::ALawEncode(uint8* dst, int16* src, uint srcSize)
{
	srcSize >>= 1;
...
EXPORT uint G711::ALawDecode(int16* dst, const uint8* src, uint srcSize)
{
...
EXPORT uint G711::ULawEncode(uint8* dst, int16* src, uint srcSize)
{
	srcSize >>= 1;
	uint8* end = dst+srcSize;
	while(dst<end)
...
EXPORT uint G711::ULawDecode(int16* dst, const uint8* src, uint srcSize)
{
	int16* end = dst+srcSize;
	while(dst<end)
		*dst++ = ULawDecode(*src++);
...
EXPORT uint G711::ALawToULaw(uint8* dst, const uint8* src, uint srcSize)
{
	uint8* end = dst+srcSize;
	while(dst<end)
		*dst++ = ALawToULaw(*src++);
...
EXPORT uint G711::ULawToALaw(uint8* dst, const uint8* src, uint srcSize)
{
	uint8* end = dst+srcSize;
	while(dst<end)
		*dst++ = ULawToALaw(*src++);
